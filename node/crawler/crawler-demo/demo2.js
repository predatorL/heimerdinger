...
(async () => {
    ...
    // 点击 展开按钮
    const unfoldBtn = await iframe.$('#flag_ctrl');
    ...
    await page.screenshot({
        path: __dirname + '/images/歌曲.png',
        fullPage: true
    });

    // 写入文件
    let writerStream = fs.createWriteStream(__dirname + '/docs/歌词.txt');
    ...
    // 获取评论
    const commentList = await iframe.$$eval('.itm', eles => {
        const ctn = eles.map(v => {
